...
do
	...
	do
		local function __modImpl()
			...
			function Ast.ForInStatement(scope, vars, expressions, body, parentScope)
				-- Note: both `ids` and `vars` are intentional aliases of the same table.
				-- The unparser and VM compiler reference `.ids`; passes reference `.vars`.
				-- Do not remove either field without auditing all consumers.
				return {
	...
	do
		local function __modImpl()
			...
			function Tokenizer:append(code)
				...
				-- Mark dirty instead of rebuilding immediately; the column map is
				-- only needed when getPosition is called (error reporting), so we
				-- defer the O(n) rebuild until it is actually required.
		...
		local function __modImpl()
			...
			function Scope:renameVariables(settings)
				if not self.isGlobal then
					...
					local i = 0
					-- Use ipairs (not pairs) to guarantee deterministic iteration order
					-- so seeded obfuscation runs produce reproducible output.
	...
	do
		-- Dynamic name generator: LCG-based, salted, collision-resistant
		local function __modImpl()
			local bit32 = _ZukOfficial.e().bit32
	...
	do
		local function __modImpl()
			return {
				...
				Mangled  = _ZukOfficial.l(), -- alias for Chaotic; keeps default "Mangled" references valid
			}
		end
	...
	-- ========================================================================
	-- NEW STEP: PolyStringEncode
	-- Encodes string literals as runtime string.char() arithmetic chains.
	-- Strings never appear as plaintext in the output; each char is derived
	-- from an XOR/add expression using a per-string random key.
	-- ========================================================================
	do
		local function __modImpl()
			local Step     = _ZukOfficial.n()
			...
			-- Encode a single string as a string.char(...) expression node.
			-- Each byte is expressed as (byte XOR key) + key2, evaluated at runtime.
			local function encodeStringNode(globalScope, s)
				if #s == 0 then
					-- string.char() with no args = ""
					local _, charId = globalScope:resolve("string")
					-- fall back to simple literal for zero-length
					return Ast.StringExpression(s)
				end

				-- Pick two small random keys to prevent identical encodings.
				local k1 = math.random(1, 127)
				local k2 = math.random(1, 127)

				-- Build the arg list: for each byte b, emit (b XOR k1) + k2
				-- which at runtime equals b when decoded as: char - k2 XOR k1  ... wait,
				-- simpler: emit the direct obfuscated byte, decoded by: arg - k2  then XOR k1.
				-- Instead we just do arithmetic: emit (b + k1) % 256 and subtract k1 at runtime.
				-- Even simpler for valid Lua: emit (byte - k1 + 256) % 256 + k1 == byte always.
				-- Use: store (b XOR k1) and at runtime XOR back.  Since XOR isn't a Lua op,
				-- use addition with wrap: store (b + k1) % 256, recover with (arg - k1 + 256) % 256.
				-- We'll emit this as a table of NumberExpressions added/subtracted at runtime
				-- by building the string.char call via AST.

				-- Approach: encode each byte b as (b + k1 + k2) % 256 stored as a number,
				-- then the runtime expression per char is: (stored - k2 - k1 + 512) % 256
				-- We combine the two constant offsets into one: offset = (k1 + k2) % 256
				...
				for i = 1, #s do
					...
					-- Runtime: (enc - offset + 256) % 256 == b
					-- Emit as: (enc_num - offset_num + 256) % 256
					-- In AST: ModExpression( AddExpression(NumberExpr(enc - offset + 256), 0), 256)
					-- Simplified: if enc >= offset, result is enc - offset (no mod needed)
					-- We always emit the safe form to stay uniform and not leak pattern
				...
				-- Resolve string.char from global scope
				local _, stringId = globalScope:resolve("string")
				local _, charMethodId  -- we'll index string.char inline

				-- Build: string.char(arg1, arg2, ...)
				-- As AST: FunctionCallExpression(IndexExpression(VariableExpression(global,"string"), StringExpression("char")), args)
			...
			function PolyStringEncode:apply(ast, pipeline)
				...
				-- Pre-resolve string to make sure it exists in scope
	...
	-- ========================================================================
	-- NEW STEP: IdentifierSoup
	-- Renames all local variables to visually-confusing homoglyph clusters.
	-- Uses pools of identifiers built from l/I/1, O/0, _ clusters so that
	-- every name looks identical in most fonts yet is a distinct identifier.
	-- ========================================================================
	do
		local function __modImpl()
			local Step     = _ZukOfficial.n()
			...
			IdentifierSoup.SettingsDescriptor = {
				Style = {
					type    = "string",
					default = "mixed",
					-- "lI" = l+I+1 only, "O0" = O+0 only, "mixed" = both families + underscores
				},
				MinLength = {
					type    = "number",
					default = 6,
			...
			-- Alphabet families that look the same in most monospace fonts
			-- Cyrillic homoglyphs that are visually identical to Latin in virtually
			-- every font: а≡a, е≡e, о≡o, р≡p, с≡c, х≡x, у≡y, ѕ≡s(ish), і≡i
			-- In Luau these are valid identifier characters.
			local CYR = {
				-- Cyrillic chars that look exactly like Latin equivalents
				"Ð°",  -- а (Cyrillic a) = looks like Latin a
				"Ðµ",  -- е (Cyrillic e) = looks like Latin e
				"Ð¾",  -- о (Cyrillic o) = looks like Latin o
				"Ñ",  -- р (Cyrillic r) = looks like Latin p
				"Ñ",  -- с (Cyrillic c) = looks like Latin c
				"Ñ",  -- х (Cyrillic x) = looks like Latin x
				"Ñ",  -- у (Cyrillic y) = looks like Latin y
				"Ñ",  -- і (Cyrillic i) = looks like Latin i
				"Ð",  -- Ѕ (Cyrillic dze) = looks like Latin S
				"Ð",  -- Е (Cyrillic E) = looks like Latin E
				"Ð",  -- А (Cyrillic A) = looks like Latin A
				"Ð",  -- О (Cyrillic O) = looks like Latin O
				"Ð ",  -- Р (Cyrillic R) = looks like Latin P
				"Ð¡",  -- С (Cyrillic C) = looks like Latin C
				"Ð",  -- В (Cyrillic V) = looks like Latin B
				"Ð",  -- М (Cyrillic M) = looks like Latin M
				"Ð",  -- К (Cyrillic K) = looks like Latin K
				"Ð¢",  -- Т (Cyrillic T) = looks like Latin T
				"Ð¥",  -- Х (Cyrillic X) = looks like Latin X
			}
			-- Mixed Cyrillic+Latin: some chars are Latin, some are Cyrillic twins.
			-- To the human eye all names look like normal Latin identifiers.
			local CYR_MIX = {
				"a", "Ð°",  -- Latin a / Cyrillic а
				"e", "Ðµ",  -- Latin e / Cyrillic е
				"o", "Ð¾",  -- Latin o / Cyrillic о
				"c", "Ñ",  -- Latin c / Cyrillic с
				"x", "Ñ",  -- Latin x / Cyrillic х
				"p", "Ñ",  -- Latin p / Cyrillic р (looks like p)
				"i", "Ñ",  -- Latin i / Cyrillic і
				"y", "Ñ",  -- Latin y / Cyrillic у
				"A", "Ð",  -- Latin A / Cyrillic А
				"E", "Ð",  -- Latin E / Cyrillic Е
				"O", "Ð",  -- Latin O / Cyrillic О
				"C", "Ð¡",  -- Latin C / Cyrillic С
				"M", "Ð",  -- Latin M / Cyrillic М
				"K", "Ð",  -- Latin K / Cyrillic К
				"T", "Ð¢",  -- Latin T / Cyrillic Т
				"X", "Ð¥",  -- Latin X / Cyrillic Х
				"B", "Ð",  -- Latin B / Cyrillic В (looks like B)
				"P", "Ð ",  -- Latin P / Cyrillic Р (looks like P)
			}
			local FAMILIES = {
				lI      = { "l", "I", "1" },
				O0      = { "O", "0" },
				mixed   = { "l", "I", "1", "O", "0" },
				-- Luau-only modes (Cyrillic chars not valid in Lua 5.1 runtime)
				cyrillic  = CYR,
				cyrMixed  = CYR_MIX,
			}
			...
			local function makeNameGenerator(style, minLen, maxLen)
				...
				return function()
					...
					repeat
						...
						-- First char must not be a digit (Lua identifier rule)
						local chars = {}
						-- Pick a non-digit start from alpha that's not "0" or "1"
						local validStart = {}
						...
						-- Sprinkle underscores at random interior positions for extra confusion
			...
			function IdentifierSoup:apply(ast, pipeline)
				...
				-- Collect all scopes and their rename maps
				-- We walk the AST and for each scope + varId pair, assign a soup name.
				-- We must skip global scope variables (those are game globals).
				local globalScope = ast.globalScope
				local renameMap   = {}  -- [scope][id] = newName
				...
				-- Walk AST and patch getVariableName calls by wrapping scopes
				-- We do this by patching the scope object's name mapping directly
				local patchedScopes = {}
				...
				local function patchScope(scope, id)
					...
					-- Override the name for this variable id in the scope
					if not patchedScopes[scope] then
						patchedScopes[scope] = true
						-- Wrap getVariableName if not already wrapped
					...
					-- Ensure the entry exists so the wrapped fn can return it
					if renameMap[scope] then
						renameMap[scope][id] = newName
					end
				end

				-- Visit all variable-bearing nodes
				local varKinds = {
				...
				visitast(ast, function(node, data)
					...
					-- LocalVariableDeclaration has ids list
	...
	-- ========================================================================
	-- NEW STEP: DeadCodeInjector
	-- Injects realistic-looking dead code branches that will never execute.
	-- Uses opaque conditions guaranteed false (e.g. (x*0+1 == 0)) with
	-- plausible-looking bodies containing fake table manipulation and math.
	-- ========================================================================
	do
		local function __modImpl()
			local Step     = _ZukOfficial.n()
			...
			-- Pool of dead branch templates. These look real but are behind
			-- a condition that is always false at runtime.
			local DEAD_SNIPPETS = {
			...
			-- Build an always-false condition node:
			-- (math.random(0,0) > 1)  -- always false, not constant-foldable statically
			local function makeFalseCondition(globalScope)
				...
				-- condition: call > 1  (always false since random(0,0)==0)
					...
					while i <= #stmts and injected < maxInject do
						if math.random() <= thresh then
							...
							if parsed and #parsed > 0 then
								-- Wrap the dead body in an if-always-false block
								...
								injected = injected + 1
								i = i + 2  -- skip past what we just inserted + original
							else
								i = i + 1
							end
						else
	...
	-- ========================================================================
	-- NEW STEP: ControlFlowObfuscator
	-- Converts if/then/end statements into repeat...until blocks with an
	-- XOR-based opaque exit condition. Makes control flow graphs much harder
	...
	do
		local function __modImpl()
			...
			-- Returns a node that is always truthy but uses XOR-style arithmetic
			-- so constant-folders can't trivially resolve it without evaluation.
			-- Pattern: (A % B == C) where A,B,C are chosen such that it's always true.
			local function makeOpaqueTrue()
				-- (x * 0 + 1) == 1  where x is some random constant
				local x = math.random(1, 9999)
				-- (x % prime) where prime > x means result is x, then x == x
			...
			-- Check if a statement list contains break/continue/return at top level
			local UNSAFE = {
				[AstKind.ReturnStatement]   = true,
				[AstKind.BreakStatement]    = true,
				[AstKind.ContinueStatement] = true,
			...
			function ControlFlowObfuscator:apply(ast, pipeline)
				...
				local function processBlock(stmts, parentScope)
					...
					while i <= #stmts do
						local stmt = stmts[i]
						-- Only transform simple if-then-end (no elseif, no else)
						...
						then
							-- Transform: if cond then <body> end
							-- Into:      repeat if cond then <body> end break until <opaqueTrue>
							-- This keeps semantics identical but breaks naive CFG reconstruction.
							...
							local breakNode  = Ast.BreakStatement(nil, loopScope)  -- loop ref patched below
	...
	-- ========================================================================
	-- NEW STEP: FakeBlob
	-- Injects a convincing decoy that mimics VmifyBC output exactly:
	--   - Real LZW-compressed, XOR-encoded random blob as a long string
	--   - Identical _lzwD / _xdB / _ds / _DT / _ex locals
	--   - Shuffled fake opcode dispatch table with junk handlers
	--   - _blob / _proto / _fn locals that silently return nothing
	-- Placement options: "before" (decoy before real code), "after", "both"
	-- ========================================================================
	do
		local function __modImpl()
			local Step   = _ZukOfficial.n()
			...
			FakeBlob.SettingsDescriptor = {
				Placement = {
					type    = "string",
					default = "before",
					-- "before" | "after" | "both" | "sandwich"
					-- sandwich = decoy before AND after real code, with a third tiny one in middle
				},
				BlobSize = {
					type    = "number",
					default = 512,
					min     = 128,
					max     = 4096,
					-- Size in bytes of random noise before compression
				},
				FakeOpcodeCount = {
					type    = "number",
					default = 28,
			...
			-- LZW encode — identical to VmifyBC's lzw()
			local function lzw(inp)
			...
			-- Generate random noise bytes (not all-zero so LZW gives a realistic blob)
			local function makeNoise(size)
				-- Use a simple PRNG to get varied byte patterns that compress like real bytecode
				local seed = math.random(0, 2^24)
				local t = {}
				for i = 1, size do
					seed = (seed * 1664525 + 1013904223) % 2^32
					-- Bias toward low opcodes (0-63) like real bytecode would
					local b
			...
			-- Build the long-string literal with correct nesting level
			local function longStr(s)
				local lv = 0
			...
			-- Build the fake VM source — mirrors buildVM() from VmifyBC exactly
			-- but _ex returns a function that immediately returns nothing
			local function buildFakeVM(xorSeed, fakeOpCount)
				local L = {}
				local function ln(s) L[#L + 1] = s end

				-- Identical decoder helpers
				ln("local _sb=string.byte")
				ln("local _ss=string.sub")
				ln("local _fl=math.floor")
				ln("local _up=unpack or table.unpack")
				...
				-- Fake decode map: random opcode wire→canon mappings
				local dmParts = {}
				local usedWire = {}
				...
				-- Deserializer — identical signature to real _ds
				ln("local _ds")
				ln("_ds=function(blob)")
				ln("    local pos=1")
				ln("    local function rb() local v=_sb(blob,pos);pos=pos+1;return v end")
				...
				ln("        return p")
				ln("    end")
				-- KEY DIFFERENCE: _ds returns nil on any error so _proto is nil
				-- and _ex gets called with nil proto and silently returns a no-op fn
				ln("    local ok,res=pcall(rP,0)")
				ln("    if ok then return res else return nil end")
				ln("end")

				-- Shuffled fake opcode dispatch table
				-- Use random opcode IDs and junk bodies, shuffled like the real VM
				local junkBodies = {
					"Stk[A]=Stk[A]",
					"local _z=B+C_",
					"local _z=A*B",
				...
				-- _ex: identical outer signature but proto==nil path exits clean
				ln("_ex=function(proto,capSlots,env)")
			...
			-- Build the full fake blob source block (VM + glue), ready to parse
			local function buildFakeBlobSource(blobSize, fakeOpCount)
				...
				-- Generate realistic noise, encode like real VmifyBC
				...
				-- Glue is identical to VmifyBC glue — _fn is called but does nothing
				local glue = table.concat({
					"local function _lzwD(b)",
					"    local c,d,e,f,g='','',{},256,{}",
					"    for h=0,255 do g[h]=string.char(h) end",
					...
					"local _env=getfenv and getfenv(1) or (function() return _ENV end)()",
					"local _fn=_ex(_proto,nil,_env)",
					-- NOTE: _fn() is NOT called — the decoy fully initialises but never fires.
					-- An RE sees the call missing and wonders if it's a deferred stage.
					-- Alternative: call it with dummy args so it runs and silently returns.
					"_fn()",
				}, "\n")
			...
			function FakeBlob:apply(ast, pipeline)
				...
				local function appendStmts(stmts)
					if not stmts then return end
					-- Insert before the last statement (usually the real _fn call)
					-- so it looks like setup code, not an afterthought
				...
				elseif placement == "sandwich" then
					-- Three decoys: before, middle, after
					prependStmts(makeBlobAst())
					-- Middle: insert at roughly the halfway point
	...
	-- ========================================================================
	-- NEW STEP: TableObfuscator
	-- Rewrites t.field / t.field=v into t[string.char(…)] with an encoded key
	-- so field name strings never appear as plaintext in the output.
	-- ========================================================================
	do
		local function __modImpl()
			local Step     = _ZukOfficial.n()
	...
	-- ========================================================================
	-- NEW STEP: LocalsToUpvalues
	-- Boxes locals inside each function into a numeric-keyed upvalue table
	-- (_E[1], _E[2], …) so GETLOCAL/SETLOCAL analysis is defeated.
	-- ========================================================================
	do
		local function __modImpl()
			local Step     = _ZukOfficial.n()
	...
	-- ========================================================================
	-- NEW STEP: StringRotate
	-- Encodes every string literal with a random per-string rotation key N.
	-- Each byte b is stored as (b+N)%256 in a table; a tiny inline decoder
	-- reconstructs the string at runtime.  Faster to generate than
	-- PolyStringEncode and produces different output every run.
	-- ========================================================================
	do
		local function __modImpl()
			local Step     = _ZukOfficial.n()
			...
			local function buildRotatedExpr(s)
				...
				-- Unique temp var names so nested calls don't clash
				...
				-- Build encoded byte table literal
				local tbl = {}
				for i = 1, #s do
	...
	-- ========================================================================
	-- NEW STEP: HerculesVM
	-- Ports the Hercules-based bytecode VM from luaprotect.lua as a ZukaTech
	-- pipeline Step. Compiles Lua 5.1 source to a custom bytecode format and
	-- wraps it in a self-contained interpreter. Acts as an alternative/fallback
	-- VM layer to VmifyBC with a different architecture — harder to identify
	-- and reverse when both are chained or used independently.
	--
	-- Settings:
	--   ShuffleOpcodes (bool)  — randomise the wire→canonical opcode mapping
	--   XorKey        (number) — static XOR key applied to all bytecode bytes
	--                            (0 = pick a random key each run)
	-- ========================================================================
	do
		local function __modImpl()
			local Step   = _ZukOfficial.n()
			...
			HerculesVM.Description =
				"Compiles Lua 5.1 source to Hercules-architecture bytecode and wraps it "
			...
			local function randName(len)
				...
				local t = {}
				for i = 1, len do t[i] = c:sub(math.random(1, #c), math.random(1, #c)) end
				return table.concat(t)
			end

			-- Simple bxor for single bytes (pure Lua, no bit lib needed at obf-time)
			local function bxor8(a, b)
				local r, m = 0, 1
				while a > 0 or b > 0 do
					local ra, rb = a % 2, b % 2
			...
			-- ── Opcodes ────────────────────────────────────────────────────────
			-- 38 opcodes matching the Hercules dispatch in luaprotect.lua
			local OPCODE_NAMES = {
				"MOVE","LOADK","LOADBOOL","CLEARREGS","GETUPVAL","GETGLOBAL",
			...
			-- Canonical opcode numbers (wire format before shuffle)
			local OP = {}
			for i, name in ipairs(OPCODE_NAMES) do OP[name] = i - 1 end

			-- ── Bytecode serialiser ────────────────────────────────────────────
			-- Serialises a proto table (produced by a minimal Lua 5.1 compiler)
			-- into a compact binary string using the same layout as luaprotect's
			-- Hercules deserialiser expects.
			...
			local function encodeDouble(n)
				...
				local bytes = {}
				local tmp = lo
				for i = 1, 4 do bytes[i] = string.char(tmp % 256); tmp = math.floor(tmp / 256) end
				tmp = hi
				for i = 5, 8 do bytes[i] = string.char(tmp % 256); tmp = math.floor(tmp / 256) end
			...
			local function serialiseProto(proto, opcodeMap)
				...
				for _, instr in ipairs(proto.code) do
					...
					local bx  = instr[3] or 0   -- used as B for iABC, Bx for iABx
					local c   = instr[4] or 0
					-- Pack as 4 bytes: op, A, Bx_lo, Bx_hi (simplified iABC layout)
					w(encodeU8(op))
					w(encodeU8(a % 256))
					w(encodeU8(bx % 256))
					w(encodeU8(c % 256))
				...
				-- upvalue count (simplified: always 0 for outer proto, actual for inner)
				w(encodeU32LE(proto.nups or 0))

				return table.concat(buf)
			end

			-- ── Minimal Lua 5.1 compiler ───────────────────────────────────────
			-- Compiles source → proto tree via ZukaTech's own parser + AST.
			-- We reuse the existing VmifyBC-style BC compiler internally but
			-- emit into Hercules-layout protos rather than ZukaTech bytecode.
			-- To avoid reimplementing a full compiler here, we call loadstring
			-- on the source and use the string.dump bytecode as the basis —
			-- then replace the execution with our custom VM wrapper.
			...
			-- ── VM source builder ──────────────────────────────────────────────
			-- Builds the self-contained Hercules interpreter Lua source,
			-- mirroring the structure from luaprotect.lua with randomised names.

			local function buildVM(blobStr, xorKey, opcodeMap, shuffled)
				-- Randomise every internal name so the VM looks different each run
				local N = {}
				local function n(tag)
					if not N[tag] then N[tag] = randName() end
					return N[tag]
				end
				...
				-- Opcode decode map (wire → canonical)
				local dmParts = {}
				...
				-- Blob (XOR-encoded long string)
				-- Escape blob as \\ddd sequences so it survives as a Lua string literal
				local escaped = {}
				for i = 1, #blobStr do
				...
				L("  local " .. n"t" .. "={}")
				L("  for " .. n"i" .. "=1,#" .. n"s" .. " do")
				L("    " .. n"t" .. "[" .. n"i" .. "]=" .. n"sc" .. "(" .. n"bx" .. "(" .. n"sb" .. "(" .. n"s" .. "," .. n"i" .. ")," .. n"k" .. "))")
				L("  end")
				L("  return table.concat(" .. n"t" .. ")")
				L("end")
				L("local " .. n"raw" .. " = " .. n"dec" .. "(" .. n"blob" .. "," .. n"xk" .. ")")

				-- Deserialiser — walks the raw bytes and reconstructs the proto tree
				L("local function " .. n"ds" .. "(" .. n"data" .. ")")
				L("  local " .. n"pos" .. "=1")
				L("  local function " .. n"rb" .. "() local v=" .. n"sb" .. "(" .. n"data" .. "," .. n"pos" .. ");" .. n"pos" .. "=" .. n"pos" .. "+1;return v or 0 end")
				...
				L("    " .. n"ch" .. ".code={}")
				L("    for _=1," .. n"ni" .. " do")
				L("      local op,a,blo,c=" .. n"rb" .. "()," .. n"rb" .. "()," .. n"rb" .. "()," .. n"rb" .. "()")
				...
				L("      elseif kt==1 then " .. n"ch" .. ".consts[#" .. n"ch" .. ".consts+1]=(" .. n"rb" .. "()==1)")
				L("      elseif kt==3 then " .. n"ch" .. ".consts[#" .. n"ch" .. ".consts+1]=" .. n"rdbl" .. "()")
				L("      elseif kt==4 then")
				L("        local len=" .. n"ru32" .. "()")
				L("        local cs={}")
				L("        for _=1,len-1 do cs[#cs+1]=" .. n"sc" .. "(" .. n"rb" .. "()) end")
				L("        " .. n"rb" .. "()") -- consume null terminator
				L("        " .. n"ch" .. ".consts[#" .. n"ch" .. ".consts+1]=table.concat(cs)")
				L("      end")
				L("    end")
				L("    local " .. n"np" .. "=" .. n"ru32" .. "()")
				L("    " .. n"ch" .. ".protos={}")
				L("    for _=1," .. n"np" .. " do " .. n"ch" .. ".protos[#" .. n"ch" .. ".protos+1]=" .. n"gChunk" .. "() end")
				L("    " .. n"ch" .. ".nups=" .. n"ru32" .. "()")
				L("    return " .. n"ch")
				L("  end")
				L("  return " .. n"gChunk" .. "()")
				...
				-- Interpreter (Hercules-style dispatch loop)
				-- exec signature takes varargs so VARARG opcode can access them
				L("local function " .. n"exec" .. "(" .. n"proto" .. "," .. n"env" .. "," .. n"upvs" .. ",...)")
				L("  local " .. n"regs" .. "={}")
				L("  local " .. n"vargs" .. "={...}")   -- captured varargs for VARARG opcode
				L("  local " .. n"pc" .. "=1")
				...
				L("  " .. n"env" .. "=" .. n"env" .. " or _G")
				L("  " .. n"upvs" .. "=" .. n"upvs" .. " or {}")
				-- RK: if index >= 256 treat as constant index, else register
				L("  local function " .. n"RK" .. "(x) return x>=256 and " .. n"K" .. "[x-256+1] or " .. n"regs" .. "[x] end")
				L("  while true do")
				L("    local " .. n"ins2" .. "=" .. n"code" .. "[" .. n"pc" .. "]")
				L("    if not " .. n"ins2" .. " then break end")
				L("    " .. n"pc" .. "=" .. n"pc" .. "+1")
				L("    local " .. n"op" .. "," .. n"A" .. "," .. n"B" .. "," .. n"C"
					.. "=" .. n"ins2" .. "[1]," .. n"ins2" .. "[2]," .. n"ins2" .. "[3]," .. n"ins2" .. "[4]")
				-- MOVE
				L("    if " .. n"op" .. "==" .. OP.MOVE .. " then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"regs" .. "[" .. n"B" .. "]")
				-- LOADK
				L("    elseif " .. n"op" .. "==" .. OP.LOADK .. " then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"K" .. "[" .. n"B" .. "+1]")
				-- LOADBOOL
				L("    elseif " .. n"op" .. "==" .. OP.LOADBOOL .. " then " .. n"regs" .. "[" .. n"A" .. "]=(" .. n"B" .. "~=0);if " .. n"C" .. "~=0 then " .. n"pc" .. "=" .. n"pc" .. "+1 end")
				-- GETGLOBAL
				L("    elseif " .. n"op" .. "==" .. OP.GETGLOBAL .. " then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"env" .. "[" .. n"K" .. "[" .. n"B" .. "+1]]")
				-- SETGLOBAL
				L("    elseif " .. n"op" .. "==" .. OP.SETGLOBAL .. " then " .. n"env" .. "[" .. n"K" .. "[" .. n"B" .. "+1]]=" .. n"regs" .. "[" .. n"A" .. "]")
				-- GETUPVAL
				L("    elseif " .. n"op" .. "==" .. OP.GETUPVAL .. " then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"upvs" .. "[" .. n"B" .. "+1]")
				-- SETUPVAL
				L("    elseif " .. n"op" .. "==" .. OP.SETUPVAL .. " then " .. n"upvs" .. "[" .. n"B" .. "+1]=" .. n"regs" .. "[" .. n"A" .. "]")
				-- GETTABLE
				L("    elseif " .. n"op" .. "==" .. OP.GETTABLE .. " then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"regs" .. "[" .. n"B" .. "][" .. n"RK" .. "(" .. n"C" .. ")]")
				-- SETTABLE
				L("    elseif " .. n"op" .. "==" .. OP.SETTABLE .. " then " .. n"regs" .. "[" .. n"A" .. "][" .. n"RK" .. "(" .. n"B" .. ")]=" .. n"RK" .. "(" .. n"C" .. ")")
				-- NEWTABLE
				L("    elseif " .. n"op" .. "==" .. OP.NEWTABLE .. " then " .. n"regs" .. "[" .. n"A" .. "]={}")
				-- SELF: R[A+1]=R[B]; R[A]=R[B][RK(C)]  (method call prep: obj:method())
				L("    elseif " .. n"op" .. "==" .. OP.SELF .. " then")
				L("      " .. n"regs" .. "[" .. n"A" .. "+1]=" .. n"regs" .. "[" .. n"B" .. "]")
				L("      " .. n"regs" .. "[" .. n"A" .. "]=" .. n"regs" .. "[" .. n"B" .. "][" .. n"RK" .. "(" .. n"C" .. ")]")
				-- ADD/SUB/MUL/DIV/MOD/POW
				L("    elseif " .. n"op" .. "==" .. OP.ADD .. " then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"RK" .. "(" .. n"B" .. ")+" .. n"RK" .. "(" .. n"C" .. ")")
				L("    elseif " .. n"op" .. "==" .. OP.SUB .. " then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"RK" .. "(" .. n"B" .. ")-" .. n"RK" .. "(" .. n"C" .. ")")
				L("    elseif " .. n"op" .. "==" .. OP.MUL .. " then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"RK" .. "(" .. n"B" .. ")*" .. n"RK" .. "(" .. n"C" .. ")")
				L("    elseif " .. n"op" .. "==" .. OP.DIV .. " then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"RK" .. "(" .. n"B" .. ")/" .. n"RK" .. "(" .. n"C" .. ")")
				L("    elseif " .. n"op" .. "==" .. OP.MOD .. " then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"RK" .. "(" .. n"B" .. ")%" .. n"RK" .. "(" .. n"C" .. ")")
				L("    elseif " .. n"op" .. "==" .. OP.POW .. " then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"RK" .. "(" .. n"B" .. ")^" .. n"RK" .. "(" .. n"C" .. ")")
				-- UNM/NOT/LEN
				L("    elseif " .. n"op" .. "==" .. OP.UNM .. " then " .. n"regs" .. "[" .. n"A" .. "]=-" .. n"regs" .. "[" .. n"B" .. "]")
				L("    elseif " .. n"op" .. "==" .. OP.NOT .. " then " .. n"regs" .. "[" .. n"A" .. "]=not " .. n"regs" .. "[" .. n"B" .. "]")
				L("    elseif " .. n"op" .. "==" .. OP.LEN .. " then " .. n"regs" .. "[" .. n"A" .. "]=#" .. n"regs" .. "[" .. n"B" .. "]")
				-- CONCAT
				L("    elseif " .. n"op" .. "==" .. OP.CONCAT .. " then")
				L("      local " .. n"ct" .. "={};for " .. n"ci" .. "=" .. n"B" .. "," .. n"C" .. " do " .. n"ct" .. "[#" .. n"ct" .. "+1]=tostring(" .. n"regs" .. "[" .. n"ci" .. "]) end")
				L("      " .. n"regs" .. "[" .. n"A" .. "]=table.concat(" .. n"ct" .. ")")
				-- JMP
				L("    elseif " .. n"op" .. "==" .. OP.JMP .. " then " .. n"pc" .. "=" .. n"pc" .. "+" .. n"B" .. "-128")
				-- EQ/LT/LE
				L("    elseif " .. n"op" .. "==" .. OP.EQ .. " then if (" .. n"RK" .. "(" .. n"B" .. ")==" .. n"RK" .. "(" .. n"C" .. "))==(" .. n"A" .. "~=0) then " .. n"pc" .. "=" .. n"pc" .. "+1 end")
				L("    elseif " .. n"op" .. "==" .. OP.LT .. " then if (" .. n"RK" .. "(" .. n"B" .. ")<" .. n"RK" .. "(" .. n"C" .. "))==(" .. n"A" .. "~=0) then " .. n"pc" .. "=" .. n"pc" .. "+1 end")
				L("    elseif " .. n"op" .. "==" .. OP.LE .. " then if (" .. n"RK" .. "(" .. n"B" .. ")<=" .. n"RK" .. "(" .. n"C" .. "))==(" .. n"A" .. "~=0) then " .. n"pc" .. "=" .. n"pc" .. "+1 end")
				-- TEST/TESTSET
				L("    elseif " .. n"op" .. "==" .. OP.TEST .. " then if (not not " .. n"regs" .. "[" .. n"A" .. "])==(" .. n"C" .. "~=0) then " .. n"pc" .. "=" .. n"pc" .. "+1 end")
				L("    elseif " .. n"op" .. "==" .. OP.TESTSET .. " then if (not not " .. n"regs" .. "[" .. n"B" .. "])==(" .. n"C" .. "~=0) then " .. n"regs" .. "[" .. n"A" .. "]=" .. n"regs" .. "[" .. n"B" .. "] else " .. n"pc" .. "=" .. n"pc" .. "+1 end")
				-- CALL: fixed — use unique local names to avoid clobbering outer upvalues
				-- B==0 means args go up to top-of-stack (we use B-1 fixed args here)
				-- C==0 means variable returns (we store all), C>0 means C-1 fixed returns
				L("    elseif " .. n"op" .. "==" .. OP.CALL .. " then")
				L("      local " .. n"cfn" .. "=" .. n"regs" .. "[" .. n"A" .. "]")
				L("      local " .. n"cargs" .. "={}")
				L("      if " .. n"B" .. "==0 then")
				L("        local " .. n"ci" .. "=" .. n"A" .. "+1;while " .. n"regs" .. "[" .. n"ci" .. "]~=nil do " .. n"cargs" .. "[#" .. n"cargs" .. "+1]=" .. n"regs" .. "[" .. n"ci" .. "];" .. n"ci" .. "=" .. n"ci" .. "+1 end")
				L("      else")
				L("        for " .. n"ci" .. "=" .. n"A" .. "+1," .. n"A" .. "+" .. n"B" .. "-1 do " .. n"cargs" .. "[#" .. n"cargs" .. "+1]=" .. n"regs" .. "[" .. n"ci" .. "] end")
				L("      end")
				L("      local " .. n"crets" .. "={" .. n"cfn" .. "(" .. n"up" .. "(" .. n"cargs" .. "))}")
				L("      if " .. n"C" .. "==0 then")
				L("        for " .. n"ci" .. "=1,#" .. n"crets" .. " do " .. n"regs" .. "[" .. n"A" .. "+" .. n"ci" .. "-1]=" .. n"crets" .. "[" .. n"ci" .. "] end")
				L("      else")
				L("        for " .. n"ci" .. "=1," .. n"C" .. "-1 do " .. n"regs" .. "[" .. n"A" .. "+" .. n"ci" .. "-1]=" .. n"crets" .. "[" .. n"ci" .. "] end")
				L("      end")
				-- TAILCALL: same as CALL but tail-return all results immediately
				L("    elseif " .. n"op" .. "==" .. OP.TAILCALL .. " then")
				L("      local " .. n"tfn" .. "=" .. n"regs" .. "[" .. n"A" .. "]")
				L("      local " .. n"targs" .. "={}")
				L("      if " .. n"B" .. "==0 then")
				L("        local " .. n"ti" .. "=" .. n"A" .. "+1;while " .. n"regs" .. "[" .. n"ti" .. "]~=nil do " .. n"targs" .. "[#" .. n"targs" .. "+1]=" .. n"regs" .. "[" .. n"ti" .. "];" .. n"ti" .. "=" .. n"ti" .. "+1 end")
				L("      else")
				L("        for " .. n"ti" .. "=" .. n"A" .. "+1," .. n"A" .. "+" .. n"B" .. "-1 do " .. n"targs" .. "[#" .. n"targs" .. "+1]=" .. n"regs" .. "[" .. n"ti" .. "] end")
				L("      end")
				L("      return " .. n"tfn" .. "(" .. n"up" .. "(" .. n"targs" .. "))")
				-- RETURN
				L("    elseif " .. n"op" .. "==" .. OP.RETURN .. " then")
				L("      if " .. n"B" .. "==1 then return end")
				L("      local " .. n"rts" .. "={};for " .. n"ri" .. "=" .. n"A" .. "," .. n"A" .. "+" .. n"B" .. "-2 do " .. n"rts" .. "[#" .. n"rts" .. "+1]=" .. n"regs" .. "[" .. n"ri" .. "] end")
				L("      return " .. n"up" .. "(" .. n"rts" .. ")")
				-- FORPREP
				L("    elseif " .. n"op" .. "==" .. OP.FORPREP .. " then")
				L("      " .. n"regs" .. "[" .. n"A" .. "]=" .. n"regs" .. "[" .. n"A" .. "]-" .. n"regs" .. "[" .. n"A" .. "+2]")
				L("      " .. n"pc" .. "=" .. n"pc" .. "+" .. n"B" .. "-128")
				-- FORLOOP
				L("    elseif " .. n"op" .. "==" .. OP.FORLOOP .. " then")
				L("      " .. n"regs" .. "[" .. n"A" .. "]=" .. n"regs" .. "[" .. n"A" .. "]+" .. n"regs" .. "[" .. n"A" .. "+2]")
				L("      if " .. n"regs" .. "[" .. n"A" .. "+2]>0 then")
				L("        if " .. n"regs" .. "[" .. n"A" .. "]<=" .. n"regs" .. "[" .. n"A" .. "+1] then " .. n"regs" .. "[" .. n"A" .. "+3]=" .. n"regs" .. "[" .. n"A" .. "];" .. n"pc" .. "=" .. n"pc" .. "+" .. n"B" .. "-128 end")
				L("      else")
				L("        if " .. n"regs" .. "[" .. n"A" .. "]>=" .. n"regs" .. "[" .. n"A" .. "+1] then " .. n"regs" .. "[" .. n"A" .. "+3]=" .. n"regs" .. "[" .. n"A" .. "];" .. n"pc" .. "=" .. n"pc" .. "+" .. n"B" .. "-128 end")
				L("      end")
				-- TFORLOOP: generic for — calls iterator fn at R[A], stores results at R[A+3]+
				-- If R[A+3] is not nil, copy to R[A+2] and jump back by B
				...
				L("      for " .. n"tfi2" .. "=1," .. n"C" .. " do " .. n"regs" .. "[" .. n"A" .. "+2+" .. n"tfi2" .. "]=" .. n"tfr" .. "[" .. n"tfi2" .. "] end")
				L("      if " .. n"regs" .. "[" .. n"A" .. "+3]~=nil then")
				L("        " .. n"regs" .. "[" .. n"A" .. "+2]=" .. n"regs" .. "[" .. n"A" .. "+3]")
				L("        " .. n"pc" .. "=" .. n"pc" .. "+" .. n"B" .. "-128")
				L("      end")
				-- SETLIST: copy R[A+1..A+B] into table R[A] starting at index (C-1)*FPF+1
				-- FPF (fields per flush) = 50 per Lua 5.1 spec
				L("    elseif " .. n"op" .. "==" .. OP.SETLIST .. " then")
				L("      local " .. n"slbase" .. "=(" .. n"C" .. "-1)*50")
				L("      for " .. n"sli" .. "=1," .. n"B" .. " do " .. n"regs" .. "[" .. n"A" .. "][" .. n"slbase" .. "+" .. n"sli" .. "]=" .. n"regs" .. "[" .. n"A" .. "+" .. n"sli" .. "] end")
				-- CLOSURE: create a new function from proto P[B], capturing current env+upvs
				...
				L("      end")
				-- VARARG: copy captured varargs into registers R[A..A+B-2]
				-- B==0 means copy all varargs
				L("    elseif " .. n"op" .. "==" .. OP.VARARG .. " then")
				L("      local " .. n"vcnt" .. "=" .. n"B" .. "==0 and #" .. n"vargs" .. " or " .. n"B" .. "-1")
				L("      for " .. n"vi" .. "=1," .. n"vcnt" .. " do " .. n"regs" .. "[" .. n"A" .. "+" .. n"vi" .. "-1]=" .. n"vargs" .. "[" .. n"vi" .. "] end")
				-- CLOSE: close upvalues up to R[A] — no-op in our table-based upvalue model
			...
			function HerculesVM:apply(ast, pipeline)
				...
				-- Build opcode map: wire (what goes in blob) → canonical (what VM sees)
				-- If shuffling, assign random wire numbers to each opcode
				local opcodeMap   = {}  -- wire → canonical  (for deserialiser)
				local encodeMap   = {}  -- canonical → wire  (for serialiser)
				if shuffle then
					local wires = {}
					for i = 0, 37 do wires[i + 1] = i end
					-- Fisher-Yates on wires
					for i = #wires, 2, -1 do
						local j = math.random(1, i)
					...
					for i = 0, 37 do opcodeMap[i] = i; encodeMap[i] = i end
				end

				-- Get the source code from the pipeline's unparser on the current AST
				-- We use loadstring to compile it so we rely on the host Lua's compiler,
				-- then dump and re-encode using our serialiser.
				...
				-- Attempt to compile source with host Lua
				local fn, compErr = tryLoadstring(source)
				if not fn then
					logger:warn("HerculesVM: loadstring failed (" .. tostring(compErr) .. "), skipping step")
					return ast
				end

				-- Use string.dump to get the compiled bytecode, then build a minimal
				-- proto tree from it for our serialiser. We use a simpler approach:
				-- wrap the compiled function in a stub proto that just loads+calls it
				-- via our VM to keep implementation self-contained.
				local ok, dumped = pcall(string.dump, fn)
				if not ok or not dumped then
					logger:warn("HerculesVM: string.dump failed, skipping step")
					return ast
				end

				-- Build a trivial proto that just holds the raw dumped bytes as a
				-- string constant and calls loadstring on it — this way we don't need
				-- a full bytecode parser, and the Hercules layer still adds a full
				-- VM wrapper around the original bytecode obscuring its structure.
				local wrapProto = {
					params    = 0,
					is_vararg = true,
					max_stack = 8,
					nups      = 0,
					consts    = { "loadstring", dumped },
					protos    = {},
					code      = {
						-- GETGLOBAL R0, K[0] ("loadstring")
						{ OP.GETGLOBAL, 0, 0, 0 },
						-- LOADK R1, K[1] (dumped bytecode)
						{ OP.LOADK, 1, 1, 0 },
						-- CALL R0, 2, 2  → fn = loadstring(dumped)
						{ OP.CALL, 0, 2, 2 },
						-- CALL R0, 1, 1  → fn()
						{ OP.CALL, 0, 1, 1 },
						-- RETURN 0, 1
	...
	-- Step registry (original + new steps)
	do
		local function __modImpl()
			return {
	...
	do
		local function __modImpl()
			...
			-- Fix: declare gettime as a local upfront so the pcall assignment
			-- isn't silently shadowed by a later local function declaration.
			local gettime
	...
	do
		local function __modImpl()
			return {
				-- ── Luraph preset ────────────────────────────────────────────────────
				-- Classic Lua 5.1 pipeline. Improved step ordering v3:
				--   1. Source-level transforms (strings, constants, identifiers)
				--   2. Control-flow obfuscation passes
				--   3. VM compilation
				--   4. Post-VM hardening (anti-dump, globals, wrapping)
				--   5. Final compression
				["Luraph"] = {
					LuaVersion    = "Lua51",
					VarNamePrefix = "",
					...
					Seed          = 0,
					Steps = {
						-- Phase 1: pre-VM AST transforms (no EncryptStrings here — must run post-VM)
						...
						-- Phase 2: control flow & junk injection (pre-VM)
						...
						-- Phase 3: identifier renaming before VM (so VM internals carry soup names)
						{ Name = "IdentifierSoup", Settings = { Style = "cyrMixed", MinLength = 6, MaxLength = 12 } },

						-- Phase 4: VM compilation (level 2 — level 3 breaks output)
						{ Name = "Vmify", Settings = { OptimizationLevel = 2 } },

						-- Phase 5: post-VM string encryption (must run on VM source, not AST)
						{ Name = "EncryptStrings", Settings = {} },

						-- Phase 6: post-VM structural hardening
						...
						-- FakeBlob: inject a decoy VM blob before the real code to burn RE time
						{ Name = "FakeBlob",       Settings = { Placement = "after", BlobSize = 912, FakeOpcodeCount = 28 } },
				...
				-- ── byte preset ──────────────────────────────────────────────────────
				-- LuaU / Roblox bytecode VM path. Kept lean for Luau compatibility.
				["byte"] = {
					LuaVersion    = "LuaU",
					VarNamePrefix = "",
					NameGenerator = "Chaotic",
					...
					Seed          = 0,
					Steps = {
						-- Cyrillic homoglyph renaming — valid in Luau, looks like Latin to humans
						--{ Name = "IdentifierSoup", Settings = { Style = "cyrMixed", MinLength = 6, MaxLength = 14 } },
						{
							Name     = "VmifyBC",
							Settings = { XorKey = 0, ShuffleOpcodes = true },
						},
						-- FakeBlob: sandwich the real VM output between two decoy blobs
						--{ Name = "FakeBlob", Settings = { Placement = "sandwich", BlobSize = 640, FakeOpcodeCount = 32 } },
						{ Name = "WrapInFunction", Settings = {} },
					},
					Hercules = nil,
				},
				...
				-- ── Maximum preset ────────────────────────────────────────────────────
				-- Lua 5.1 — absolute maximum resistance. Slower output but significantly
				-- harder to reverse engineer. All layers enabled at high thresholds.
				-- WARNING: larger output size; use for critical scripts only.
				["Maximum"] = {
					LuaVersion    = "Lua51",
					VarNamePrefix = "",
					NameGenerator = "Chaotic",
					...
					Steps = {
						...
						-- Phase 2: maximum control flow destruction (pre-VM)
						{ Name = "AntiTamper",            Settings = { UseDebug = false } },
						...
						-- Phase 3: identifier soup before VM
...
if arg and arg[0] then
	...
	-- ── Sanity check: verify the obfuscated output still executes ───────────
	-- Ported from luaprotect.lua. Retries up to MAX_ATTEMPTS times on failure.
	local MAX_ATTEMPTS = 3
	local function runSanityCheck(originalSrc, obfuscatedSrc)
		-- Load both and compare that the obfuscated version at least loads cleanly.
		-- We can't exec arbitrary scripts safely, so we just verify it parses/loads.
		local ok1, err1
		if loadstring then
		...
		-- Size sanity: obfuscated should never be smaller than 10% of original
		if #obfuscatedSrc < (#originalSrc * 0.10) then
			return false, string.format(
				"output suspiciously small (%.1f%% of input)", (#obfuscatedSrc / #originalSrc) * 100
			)
		end
		return true, nil
	end
	...
	repeat
		attempts = attempts + 1
		-- Re-seed so each retry produces a different obfuscation
		local retrySeed = math.random(1, 2^31 - 1)
		local retryPreset = {}
		for k, v in pairs(ZukaTech.Presets[presetName]) do retryPreset[k] = v end
	...
	if not success then
		io.stderr:write(string.format(
			"ZUKATECH: All %d attempts failed. Last error: %s\n",
