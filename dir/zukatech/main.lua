...
do
	...
	do
		local function __modImpl()
			...
			-- Build a per-build shuffled opcode pool so every obfuscation run
			-- produces a completely different set of state machine IDs.
			-- This prevents pattern-matching attacks across multiple outputs.
			local _opPool = (function()
				local POOL_SIZE = 512  -- larger pool = more unique IDs before reuse
				local pool = {}
				local used = {}
				-- Fill with unique randoms spread across a wide range
			...
			local function randOp()
				...
				-- XOR with a per-call random to add a second layer of variance
				-- so even if the pool order is guessed, values stay unpredictable
		...
		local function __modImpl()
			...
			local Pipeline = {
				...
				DefaultSettings = {
					...
					PrettyPrint = false,
					Seed = math.random(1, 2^31 - 1), -- dynamic per-process seed
					VarNamePrefix = "",
				},
			}
		...
		local function __modImpl()
			return {
				["Luraph"] = {
					...
					PrettyPrint = false,
					Seed = math.random(1, 2^31 - 1), -- unique seed every obfuscation run
					Steps = {
						-- PHASE 1: Structural transforms first (no string injection yet)
						{
						...
						-- PHASE 2: Anti-analysis layers
						{ Name = "AntiTamper", Settings = { UseDebug = false } },
						{ Name = "AntiDump", Settings = { GCInterval = 50 } },

						-- PHASE 3: Encryption & XOR (these inject loadstring/pcall stubs)
						{ Name = "DynamicXOR", Settings = { Treshold = 0.80 } },
						...
						-- PHASE 5: VM layer (heaviest protection)
						{ Name = "Vmify", Settings = { OptimizationLevel = 2 } },

						-- PHASE 6: Number & constant obfuscation
						{ Name = "NumbersToExpressions", Settings = { Treshold = 0.8, InternalTreshold = 0.1 } },
						{
							...
							Settings = {
								...
								MockStringChance = 8,
								MinAbsValue = 1, -- catch ALL values including small ones
							},
						},

						-- PHASE 7: Global virtualization
						{ Name = "VirtualGlobals", Settings = { Treshold = 1, UseNumericKeys = true } },

						-- PHASE 8: Second VM pass — re-VMifies the first VM's interpreter loop
						-- so a reverser must break two nested VMs instead of one.
						-- OptimizationLevel 1 keeps output size from ballooning.
						{ Name = "Vmify", Settings = { OptimizationLevel = 1 } },

						-- PHASE 10: Wrap, then encrypt ALL remaining strings
						-- (catches loadstring/assert/pcall injected by earlier steps)
						{ Name = "WrapInFunction", Settings = {} },
						{ Name = "EncryptStrings", Settings = {} }, -- FIRST pass: catches injected stubs

						-- PHASE 11: Second constants pass (catches anything EncryptStrings re-exposed)
						{
							Name = "ConstantsObfuscator",
							Settings = {
								ObfuscateNumbers = true,
						...
						-- PHASE 12: Final compression with MinLength=1 to catch ALL short strings
						-- (e.g. "assert", "pcall", "load" are all < 10 chars and were previously skipped)
						{ Name = "Compressor", Settings = { MinLength = 1 } },

						-- PHASE 13: Second EncryptStrings pass after compression
						{ Name = "EncryptStrings", Settings = {} }, -- SECOND pass: catches anything Compressor exposed

						-- PHASE 14: Dead code cleanup last so it doesn't remove our junk
						{
